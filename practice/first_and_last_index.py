...
def first_and_last(array:list,target:Any)-> list:
    
    for i in range(len(array)):
        if arr[i] == target:
            start = i
            
            while i+1<len(array) and arr[i+1] == target:
                i += 1
...
#O(log(n)) time, O(1)
def first_and_last_binary_search(array:list,target:Any)-> list:
...
#O(log(n)) time for binary search, O(1)
     ...
     def find_start(array:list,target:Any)-> int:
     ...
     def find_end(array:list,target:Any)-> int:
        left,right = 0,len(array)-1
        if arr[-1]==target:
            return -1
        while left<=right:
